...
def _public(profile: dict[str, Any]) -> dict[str, Any]:
    fields = ('user_id', 'display_name', 'gender', 'seeking', 'intent', 'age', 'city',
              'bio', 'hobbies', 'values', 'languages', 'personality', 'lifestyle',
              'deal_breakers')
    # .get() để hồ sơ cũ thiếu field mới vẫn đọc được thay vì KeyError.
...
def search_candidates(city: str, hobbies: str | list[str], age_min: int, age_max: int,
                      gender: str = '', exclude_user_id: str = '') -> str:
    '''Lọc ứng viên theo city, hobbies, tuổi và (tùy chọn) gender.

    hobbies nhận list[str] hoặc chuỗi phân tách dấu phẩy. gender nhận
    nam | nữ | khác, để trống nghĩa là không lọc theo giới tính.
    exclude_user_id loại chính người đang đăng nhập khỏi kết quả.
    Kết quả gồm criteria, count, candidates. Tool read-only; input sai trả JSON lỗi.
    '''
    try:
        if not isinstance(city, str) or not city.strip():
            return _out(code='INVALID_ARGUMENT', message='city phải là chuỗi không rỗng.')
        if not isinstance(gender, str):
            return _out(code='INVALID_ARGUMENT', message='gender phải là chuỗi.')
        gender_filter = _norm(gender) if gender.strip() else ''
        if gender_filter and gender_filter not in {_norm(g) for g in GENDERS}:
            return _out(code='INVALID_ARGUMENT',
                        message='gender chỉ nhận {} hoặc để trống.'.format(' | '.join(GENDERS)))
        ...
        excluded = exclude_user_id.strip().upper() if isinstance(exclude_user_id, str) else ''
        results = []
        for profile in data['profiles']:
            ...
            if wanted and not matches:
                continue
            if gender_filter and _norm(profile.get('gender') or '') != gender_filter:
                continue
            if excluded and profile.get('user_id', '').upper() == excluded:
                continue
        ...
        response = {'criteria': {'city': city, 'hobbies': items, 'age_min': low,
                                 'age_max': high, 'gender': gender or 'không lọc'},
